...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      <header className="absolute inset-x-0 top-0 z-10 text-white">
        <div className="container flex h-20 items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a className="hidden text-sm font-medium text-muted-foreground hover:text-foreground sm:inline" href={isAuthenticated ? '/dashboard' : getLoginUrl()}>{isAuthenticated ? 'Dashboard' : 'Log in'}</a>
            <Button size="sm" asChild><a href={isAuthenticated ? '/dashboard' : getLoginUrl()}>{isAuthenticated ? 'View portfolio' : 'Start investing'} <ArrowRight data-icon="inline-end" /></a></Button>
          </div>
        </div>
      </header>
        ...
        <div className="container grid items-center gap-16 pb-20 lg:grid-cols-[1.08fr_.92fr] lg:pb-28">
          <div className="max-w-2xl">
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <Button size="lg" className="bg-emerald-300 text-emerald-950 hover:bg-emerald-200" asChild><a href={isAuthenticated ? '/dashboard' : getLoginUrl()}>{isAuthenticated ? 'Open your dashboard' : 'Create your account'} <ArrowRight data-icon="inline-end" /></a></Button>
      ...
      <section className="container py-24 text-center sm:py-32"><Globe2 className="mx-auto mb-6 text-primary" /><h2 className="mx-auto max-w-3xl text-4xl font-semibold tracking-tight sm:text-6xl">Your next chapter starts with one decision.</h2><p className="mx-auto mt-6 max-w-xl text-lg leading-8 text-muted-foreground">Build a portfolio that reflects where you are going, not just where you have been.</p><Button size="lg" className="mt-8" asChild><a href={isAuthenticated ? '/dashboard' : getLoginUrl()}>{isAuthenticated ? 'Go to dashboard' : 'Start investing today'} <ArrowRight data-icon="inline-end" /></a></Button></section>
