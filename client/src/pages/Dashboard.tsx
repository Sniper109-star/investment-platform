...
export default function Dashboard() {
  ...
  return <DashboardLayout>
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
      ...
      <Card><CardHeader><CardTitle>Available plans</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{(plans.data ?? []).map((plan: any) => <div key={plan.id} className="rounded-lg border p-4"><div className="flex items-start justify-between gap-2"><p className="font-semibold">{plan.name}</p><CheckCircle2 className="text-emerald-600" /></div><p className="text-sm text-muted-foreground">{plan.roi}% ROI · {plan.duration} days</p><p className="mt-2 text-sm">From ${Number(plan.minAmount).toLocaleString()}</p></div>)}</CardContent></Card>
    </div>
  </DashboardLayout>;
}
